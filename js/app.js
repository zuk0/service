...
app = {
    ...
    */
    select: function(state){
        ...
        this.dom.panel.style.display = this.path[0]? 'block' : 'none';
    },
    /* @method: build
     *
    ...
    */
    ...
    createAppFrame: function(){
        ...
        this.dom[data.place].appendChild(el);
        data.ready = true;
        console.log("@app frame created", this.dom[data.place].firstElementChild);
    },
    deleteAppFrame: function(){
        this.dom[this.frame.place].removeChild(_byId(this.frame.id));        
    },
    /* @method: enter
     *
    ...
    enter: function(state, target){
        ...
        switch(state.id){
            ...
            case 'actions':
                ...
                if(!this.frame.focus){
                    this.frame.focus = function(){
                        console.warn("lets focus on test...");
                        this.dom.update('mode','loaded', true);
                        _byId('tests').style.display = 'block';
                        return this.state.current('tests').go();
                    }.bind(this);
                }
                if(!this.frame.ready){
                    this.createAppFrame();
                ...
                //return this.frame.ready? _crateAppFrame(this.frame) : this.load(type);
                //this.load(type, this.frame.ready? this.frame : _crateAppFrame(this.frame));
                //this.frame.ready? this.load(type) : this.load(type,_crateAppFrame(this.frame));
            break;
            case '_app':
                console.log("@APP ENTER ON "+state.id);
                var frame = _byId('new_app');
    ...
    */
    init: function(state, mode){
        ...
        this.dom.update('mode',mode || 'idle',true);
